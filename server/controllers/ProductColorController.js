...
class ProductColorController {
    ...
    async addColor(req, res) {
        try {
            const { productID, colorName, images } = req.body;
    ...
    async deleteColor(req, res) {
        try {
            ...
            // Kiểm tra có size nào của màu này không
            const hasStocks = await ProductSizeStock.exists({ colorID: id });
            if (hasStocks) {
                return res.status(400).json({
                    message: 'Không thể xóa màu này vì đang có các size sử dụng'
                });
            }
